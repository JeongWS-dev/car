...
public class PaperList extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		*/
		// 관리자(admin)으로 로그인 했을 때만 회원조회가 가능하도록 해야 한다. 
		...
		//	System.out.println("~~~~~ 확인용 currentShowPageNo : " + currentShowPageNo);
			...
			if(searchType == null || (!"fk_carname".equals(searchType) && !"fk_userid".equals(searchType)&& !"pk_paperseq".equals(searchType))) {
	    ...
	    */
			...
			// *** [맨처음][이전]만들기 *** //
			// 맨처음
			pageBar += "<li class='page-item'><a class='page-link' href='paperList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo=1'>[맨처음]</a></li>";			
			// 이전
			if(pageNo != 1) { // 1을 뺀 나머지
				pageBar += "<li class='page-item'><a class='page-link' href='paperList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+(pageNo-1)+"'>[이전]</a></li>";					
			}
			...
			while(!(loop > blockSize || pageNo > totalPage)) { // !() 의 괄호 안이 참이면 빠져 나오는 것, totalPage ==> 전체 페이지 수
				...
				if(pageNo == Integer.parseInt(currentShowPageNo)) { // 현재 페이지 번호와 내가 보고자 하는 페이지 번호가 같은지?				
					...
					pageBar += "<li class='page-item'><a class='page-link' href='paperList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>"+pageNo+"</a></li>";					
				}
			...
			if( pageNo <= totalPage) { // 맨 마지막이 아니라면 그때만 다음을 넣어준다.
				pageBar += "<li class='page-item'><a class='page-link' href='paperList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>[다음]</a></li>";					
			}
			// 마지막
			pageBar += "<li class='page-item'><a class='page-link' href='paperList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+totalPage+"'>[마지막]</a></li>";					
		...
		//	System.out.println("~~~currentURL => "+currentURL);
			...
			List<Map<String,String>> paperList = cdao.select_Paper_paging(paraMap);
			
		//	System.out.println("확인용 => "+paperList);
			...
			int totalPaperCount = cdao.getTotalPaperCount(paraMap);
			//System.out.println("~~ 확인용 " +totalMemberCount );
			request.setAttribute("totalPaperCount", totalPaperCount);
			request.setAttribute("currentShowPageNo", currentShowPageNo);
			/* 검색이 있는 또는 검색이 없는 회원의 총개수 알아오기 끝*/
			
			//super.setRedirect(false);
			super.setViewPage("/WEB-INF/CreateCar/paperList.jsp");
		}
