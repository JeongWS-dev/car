...
<style type="text/css">
  ...
  table#drivingTbl tr.drivingInfo:hover {
     background-color : #F5F5F5;
     cursor: pointer;
      color : #ff8040;
  }
 
  form[name="driving_search_frm"] {
     border: solid 0px red;
     width: 80%;
     margin: 0 auto 3% auto;
  }
 
  form[name="driving_search_frm"] button.btn-secondary {
     margin-left: 2%;
     margin-right: 32%;
  }
   ...
   $(document).ready(function(){
     ...
     <%--**** select 태그에 대한 이벤트는 click 이 아니라 change 이다. **** //.--%>
        $("select[name='sizePerPage']").bind("change", function(){
         const frm = document.driving_search_frm;
    ...
    // **** 특정 회원을 클릭하면 그 회원의 상세정보를 보여주도록 한다. **** //
       $("table#drivingTbl tr.drivingInfo").click( e => {
       ...
       // 또는
          const applyseq = $(e.target).parent().children(".applyseq").text();
          //alert(userid);
          
          const frm = document.drivingOneDetail_frm;
          frm.applyseq.value=applyseq;
          
   <%--      frm.action = "<%= ctxPath%>/member/memberOneDetail.up"; 아래와 같은 내용임 --%>
          frm.action = "${pageContext.request.contextPath}/admin/drivingOneDetail.car";
          frm.method ="post";
          frm.submit();           
       });
...
<div class="container">
  <h2 class="text-center mb-5"> &lt; 모든 시승신청 보기 &gt; </h2>
 
  <form name="driving_search_frm">
     <select name="searchType" style="height : 30px;">
        <option value="">검색대상</option>
        <option value="fk_carname">차량명</option>
        <option value="fk_userid">아이디</option>
        <option value="pk_applyseq">견적서번호</option>
     </select>
     &nbsp;
     ...
     <%--
      ...
      <input type="text" style="display: none;" /> <%-- 조심할 것은 type="hidden" 이 아니다. --%>
        ...
        <span style="font-size: 12pt; font-weight: bold;">페이지당 견적서 수&nbsp;:&nbsp;</span>
     ...
     <table class="table table-bordered" id="drivingTbl">
     <thead>
         <tr>
            <th class="hidden_cell">번호</th>
            <th>시승신청번호</th>
            <th>아이디</th>
            <th>고객명</th>
            <th>차량명</th>
     ...
     <tbody>
        <c:if test="${not empty requestScope.drivingList}">
           <c:forEach var="drivingvo" items="${requestScope.drivingList}" varStatus="status">
              <tr class="drivingInfo">
                 ...
                 <td class="hidden_cell">${(requestScope.totalDrivingCount) - (currentShowPageNo-1) * sizePerPage - (status.index)} </td>
                ...
                --%>
                 ...
                 <td class="applyseq">${drivingvo.pk_applyseq}</td><%-- get 다음의 userid 적기 --%>
                 <td>${drivingvo.fk_userid}</td>
                 <td>${drivingvo.username}</td>
                 <td>${drivingvo.fk_carname}</td>
                 <td>${drivingvo.fk_place_name}</td>
                 <td>${drivingvo.schedule}</td>
              </tr>
           </c:forEach>
        </c:if>
       <c:if test="${empty requestScope.drivingList}">
          <tr>
...
<form name="drivingOneDetail_frm">
