...
//var carSearchType = $(e.target).text() // 차 종류를 가져옴
   ...
   // console.log("~~~ 확인용 json 1=> ", JSON.stringify(json));
      ...
      v_html +=  `<div class="swiper-slide">
                  <div class="card"  style="width:90%; padding-top: 10%;">
                     <div class="vehicle_title">
                        <div class= "vehicle_name" style="font-weight:100;  font-size: 150%;">
                           \${item.Pk_CarName.length < 11 ? item.Pk_CarName.split('_').join(' ') : item.Pk_CarName.split('_', 1)[0]}`
      

      v_html += `            </div>
                        <div class="brand_title">
                           <span class="card-title" style="font-weight:100;  font-size: 100%;">`
      ...
      if(item.CarLogo != null){
         if(item.CarLogo == 'NEW'){
         v_html +=`               <img src="<%= ctxPath%>/images/Estimate/new_50x24.png" style="float: right; margin-right:5%;" />`
         }
         else if(item.CarLogo == 'EV'){
         v_html +=`               <img src="<%= ctxPath%>/images/Estimate/ev_20x24.png" style="float: right; margin-right:5%;" />`
         }
      }
      ...
      v_html += `         
                     <div class="bar_image">
                        <img src="<%= ctxPath%>/images/Estimate/\${item.Pk_CarName}.png" class="car_image" style="width : 100%; "/> 
                     </div>

                     <button type="button" class="our-models-button" onclick="goCreateCar('\${item.Pk_CarName}')">자세히보기</button>
...
//차량 선택(자세히보기) 페이지로 이동하는 함수
function goCreateCar(carname){ 
	location.href=`<%= ctxPath%>/createCar/powertrains.car?pk_carname=\${carname}`;
}
  ...
  // 새로운 Swiper 객체를 생성합니다.
   ...
   <div class = "main-page">
      ...
      <div class = "our-models" >
         ...
         <div class="slider_outline" style="border: solid 0px yellow; ">
            <div id="bar_img" class="swiper mySwiper" >
