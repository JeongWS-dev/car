...
public class MemberList extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	System.out.println("~~~~~ 확인용 currentShowPageNo : " + currentShowPageNo);
			...
			if(searchType == null || (!"username".equals(searchType) && !"pk_userid".equals(searchType) && !"useremail".equals(searchType))) {
	    ...
	    */
			...
			// *** [맨처음][이전]만들기 *** //
			// 맨처음
			pageBar += "<li class='page-item'><a class='page-link' href='memberList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo=1'>[맨처음]</a></li>";			
			// 이전
			if(pageNo != 1) { // 1을 뺀 나머지
				pageBar += "<li class='page-item'><a class='page-link' href='memberList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+(pageNo-1)+"'>[이전]</a></li>";					
			}
			...
			while(!(loop > blockSize || pageNo > totalPage)) { // !() 의 괄호 안이 참이면 빠져 나오는 것, totalPage ==> 전체 페이지 수
				...
				if(pageNo == Integer.parseInt(currentShowPageNo)) { // 현재 페이지 번호와 내가 보고자 하는 페이지 번호가 같은지?				
					...
					pageBar += "<li class='page-item'><a class='page-link' href='memberList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>"+pageNo+"</a></li>";					
				}
			...
			if( pageNo <= totalPage) { // 맨 마지막이 아니라면 그때만 다음을 넣어준다.
				pageBar += "<li class='page-item'><a class='page-link' href='memberList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>[다음]</a></li>";					
			}
			// 마지막
			pageBar += "<li class='page-item'><a class='page-link' href='memberList.car?searchType="+searchType+"&searchWord="+searchWord+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+totalPage+"'>[마지막]</a></li>";					
		...
		//	System.out.println("~~~currentURL => "+currentURL);
			...
			super.setViewPage("/WEB-INF/myPage/member/memberList.jsp");
		}
