...
<style type="text/css">
  ...
  table#paperTbl tr.paperInfo:hover {
     background-color : #F5F5F5;
     cursor: pointer;
      color : #ff8040;
  }
 
  form[name="paper_search_frm"] {
     border: solid 0px red;
     width: 80%;
     margin: 0 auto 3% auto;
  }
 
  form[name="paper_search_frm"] button.btn-secondary {
     margin-left: 2%;
     margin-right: 32%;
  }
...
<script type="text/javascript">
	$(document).ready(function(){
     ...
     <%--**** select 태그에 대한 이벤트는 click 이 아니라 change 이다. **** //.--%>
     	$("select[name='sizePerPage']").bind("change", function(){
   		const frm = document.paper_search_frm;
    ...
    // **** 특정 회원을 클릭하면 그 회원의 상세정보를 보여주도록 한다. **** //
    	$("table#paperTbl tr.paperInfo").click( e => {
    	...
    	// 또는
    		const paperseq = $(e.target).parent().children(".paperseq").text();
    		//alert(userid);
    		
    		const frm = document.paperOneDetail_frm;
    		frm.paperseq.value=paperseq;
    		
   <%--		frm.action = "<%= ctxPath%>/member/memberOneDetail.up"; 아래와 같은 내용임 --%>
    		frm.action = "${pageContext.request.contextPath}/admin/paperOneDetail.car";
    		frm.method ="post";
    		frm.submit();     		
    	});
...
<div class="paperListContainer">
  <h2 class="text-center mb-5"> &lt; 모든 견적 보기 &gt; </h2>
  ...
  <form name="paper_search_frm">
     <select name="searchType" style="height : 30px;">
        ...
        <option value="pk_paperseq">견적서번호</option>
     </select>
     &nbsp;
     ...
     <%--
  	...
  	<table class="table table-bordered" id="paperTbl">
     <thead>
         <tr>
     ...
     <tbody>
     	<c:if test="${not empty requestScope.paperList}">
     		<c:forEach var="papervo" items="${requestScope.paperList}" varStatus="status">
     			<tr class="paperInfo">
     				...
     				<td class="hidden_cell">${(requestScope.totalPaperCount) - (currentShowPageNo-1) * sizePerPage - (status.index)} </td>
                ...
                --%>
     				...
     				<td class="paperseq">${papervo.pk_paperseq}</td><%-- get 다음의 userid 적기 --%>
     				<td>${papervo.fk_userid}</td>
     				<td>${papervo.username}</td>
     				<td>${papervo.fk_carname}</td>
     				<td>${papervo.fk_incolorcode}</td>
     				<td>${papervo.fk_powercode}</td>
     				<td>${papervo.fk_outcolorcode}</td>
     			</tr>
     		</c:forEach>
     	</c:if>
       <c:if test="${empty requestScope.paperList}">
       	<tr>
...
<form name="paperOneDetail_frm">
	<input type="hidden" name="paperseq" />
