...
@WebServlet(
	      description = "사용자가 웹에서 *.car 을 했을 경우 이 서블릿이 응답을 해주도록 한다.",
	      urlPatterns = {"*.car"},
	      initParams = { 
	      @WebInitParam(name = "propertyConfig", value = "/Users/jeong-useog/Desktop/NCS/workspace_JSP/car/car/src/main/webapp/WEB-INF/Command.properties", description = "*.car 에 대한 클래스의 매핑파일") 
	      })
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// *** 확인용 *** //
		// System.out.println(">>> 확인용 Servlet FrontController의 init(ServlerConfig config) 메소드가 실행됨.");
		FileInputStream fis = null;
		// 특정 파일에 있는 내용을 읽어오기위한 용도로 쓰이는 객체
		
		String props = config.getInitParameter("propertyConfig");
		// System.out.println("확인용 : " + props);
		// 확인용 : C:\NCS\workspace_jsp\MyMVC\src\main\webapp\WEB-INF\Command.properties
		...
		try {
	        ...
	        */
			...
			while(en.hasMoreElements()) {
				String key = (String)en.nextElement(); // en 이 있으면 key값을 가져온다. return값이 object라서 String으로 형변환 시켜주어야한다
...
//					System.out.println(">>> className = " + className);
					...
					/*
					 	--- 확인용 Test2Controller 클래스 기본생성자 호출 --- 
						--- 확인용 Test3Controller 클래스 기본생성자 호출 --- 
						--- 확인용 Test1Controller 클래스 기본생성자 호출 --- 
					 */
					cmdMap.put(key, obj);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// System.out.println(">>> 확인용 Servlet FrontController의 doGet(HttpServletRequest request, HttpServletResponse response) 메소드가 실행됨.");
		...
		// /MyMVC/member/idDuplicateCheck.up 에서 /MyMVC 다음부터 따오는 substring
